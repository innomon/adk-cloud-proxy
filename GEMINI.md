...
## Testing
- Unit tests for logic should be placed alongside the code (e.g., `pkg/goose/handler_test.go`).
- The `target-server` is a mock that can be used for end-to-end testing of the proxy and connector.

## Common Tasks
- **Updating REST Routes:** Ensure any changes to the ADK REST API surface are applied to both `cmd/target-server/main.go` and `pkg/goose/handler.go`.
- **Config Changes:** Configuration is managed in `pkg/config/config.go` and loaded from `config.yaml`.
