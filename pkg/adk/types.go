...
// ADKMessage represents a message in a ADK conversation.
...
// SSEEvent represents a server-sent event from the ADK streaming API.
type SSEEvent struct {
	Type       string        `json:"type"`
	Message    *ADKMessage `json:"message,omitempty"`
...
// StartAgentRequest is the payload sent to start a new ADK agent session.
...
type ReplyRequest struct {
	UserMessage       *ADKMessage  `json:"user_message"`
	SessionID         string         `json:"session_id"`
	ConversationSoFar []ADKMessage `json:"conversation_so_far,omitempty"`
}
...
type SessionHistoryResponse struct {
	...
	Metadata  *SessionMetadata `json:"metadata,omitempty"`
	Messages  []ADKMessage   `json:"messages"`
}
